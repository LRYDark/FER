...
/* ── Empreinte d'email (clé de rattachement des comptes coureurs) ────────────
 *
 * POURQUOI UNE EMPREINTE ET NON L'ADRESSE EN CLAIR.
 * `registrations.email` est chiffré (AES-256-GCM, IV aléatoire) : deux
 * chiffrements de la même adresse donnent deux valeurs différentes. Impossible
 * donc de comparer, d'indexer ou de joindre sur la colonne chiffrée — c'est
 * pour cette raison que public/accueil.php et le chatbot déchiffrent
 * aujourd'hui TOUTE la table pour retrouver un inscrit par son adresse.
 *
 * Stocker l'adresse en clair à côté annulerait le chiffrement : une sauvegarde
 * de base exposerait toutes les adresses. On stocke donc une empreinte
 * HMAC-SHA256 (64 caractères hexadécimaux) :
 *   - déterministe  → indexable, comparable, joignable (= ce dont on a besoin) ;
 *   - non réversible → aucune adresse lisible en base ;
 *   - clé dérivée d'ENCRYPTION_KEY par un contexte dédié (séparation de clés) :
 *     l'empreinte ne peut pas servir à autre chose que ce rôle.
 *
 * ⚠️ Une empreinte ne permet PAS la recherche partielle (LIKE « @gmail.com »).
 * ⚠️ Une rotation d'ENCRYPTION_KEY impose de recalculer toutes les empreintes,
 *    exactement comme il faudrait re-chiffrer toutes les colonnes PII.
 * ⚠️ Le suffixe « :v1 » du contexte permet de changer d'algorithme un jour sans
 *    ambiguïté sur les valeurs déjà stockées.
 */

/**
 * Une colonne existe-t-elle ? (mis en cache pour la durée de la requête)
 *
 * Sert de garde aux écritures du lot 1 : le code peut être déployé avant que
 * update.php n'ait été lancé. Sans ce test, toute création d'inscription
 * échouerait sur une colonne inconnue — c'est le formulaire public qui casserait.
 */
function fer_hasColumn(PDO $pdo, string $table, string $col): bool {
    static $cache = [];
    $cle = $table . '.' . $col;
...
/**
 * Identifiant de l'édition active (`editions.is_active = 1`), ou null si la
 * table n'existe pas encore. Mis en cache : appelé à chaque création d'inscription.
 *
 * Repli volontaire sur l'édition la plus récente si aucune n'est marquée active,
 * pour ne jamais créer d'inscription orpheline : un `edition_id` NULL rendrait
 * l'unicité (edition_id, inscription_no) inopérante et l'inscription
 * n'apparaîtrait dans aucune édition de l'espace coureur.
 */
function fer_activeEditionId(PDO $pdo, bool $refresh = false): ?int {
    static $cache = false;
    if ($cache !== false && !$refresh) return $cache;

    $cache = null;
    try {
        $id = $pdo->query(
            'SELECT id FROM editions ORDER BY is_active DESC, annee DESC, id DESC LIMIT 1'
        )->fetchColumn();
        if ($id !== false && $id !== null) $cache = (int) $id;
    } catch (\Throwable $e) {
        // Table absente (migration pas encore jouée) : l'appelant n'écrit
        // simplement pas la colonne.
    }
    return $cache;
}
...
/**
 * Empreinte déterministe d'une adresse email (64 caractères hex), ou null si
 * l'adresse est vide. C'est la valeur stockée dans les colonnes
 * `email_normalise` (registrations, participants, participant_auth_codes).
 */
function fer_emailHash(?string $email): ?string {
    $norm = fer_normalizeEmail($email);
    if ($norm === '') return null;
    // Sous-clé dédiée : on n'utilise jamais directement la clé de chiffrement.
    static $subKey = null;
    if ($subKey === null) {
        $subKey = hash_hmac('sha256', 'fer:email_norm:v1', CIPHER_KEY, true);
    }
    return hash_hmac('sha256', $norm, $subKey);
}
